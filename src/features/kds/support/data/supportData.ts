...
//
// Design rule:
//   - Steps that previously had only one "확인했어요"/"알겠어요" option are now
//     converted to answer-only steps: the answer is delivered directly by the
//     bot, and the step immediately advances to "terminal" without requiring
//     the user to click a confirmation button.
//   - Options with a real label (non-confirmation) are kept as-is.
// ─────────────────────────────────────────────────────────────
...
export type QnaStep = {
  ...
  options: QnaOption[];
  /**
   * If true, the bot will deliver this step's question as an answer bubble
   * and immediately advance to `autoAdvanceTo` without showing any options.
   */
